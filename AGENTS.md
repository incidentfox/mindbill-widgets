...
The public packages are published to npm with provenance. Agents may use the CLI to create a synthetic sandbox, but must save the one-time key to a gitignored file. Managed customer organizations should be provisioned without a MindBill user or invitation by default; direct customer access is an explicit opt-in. An agent may request a Stripe Checkout URL; only a human may open it and provide payment information.
