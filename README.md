...
`RfaDashboard` provides patient/injury setup, revision-safe request editing, saved practice and authorization contacts, clinical PDF uploads, embedded physician signature setup, reviewed signing, and confirmed fax/email delivery of the reviewed packet. It also includes searchable RFA and individual-treatment views, draft copy/cancellation, packet download, delivery evidence, actor-attributed notes, decision corrections, and appointment tracking. See [RFA dashboard](docs/rfa-dashboard.md) for server scopes, signer setup, and sandbox behavior.
