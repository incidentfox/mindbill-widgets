...
Billing providers can use `taxIdType: "EIN" | "SSN"` (EIN is the default). SSN inputs are password fields. For a provider already saved in MindBill, pass `billingProvider: { savedProviderId: "synthetic_provider_id" }` instead of copying its masked tax ID into the form. Corrections and duplicates of an existing SSN bill use `{ sourceBillId: "synthetic_bill_id" }` to preserve that bill's immutable provider snapshot. The component preserves either reference through submission; MindBill resolves the sensitive identifier server-side within the same organization. A user must explicitly choose **Enter a different billing provider** before replacing that reference with manually entered fields. Existing hosts that access billing-provider fields directly should narrow the union with `"name" in bill.billingProvider` first.

`MindBillOrganizationOnboardingComponent` supports EIN and encrypted saved SSN profiles. Saved SSNs return only their last four digits and configured state: leave the input blank to preserve an existing SSN, enter a value to replace it, or use **Clear saved SSN** to request removal on the next save. Changing the tax ID type resets the input rather than silently reinterpreting the previous identifier. Save/completion events contain the returned profile, not the submitted plaintext SSN. Never log or persist manually entered sensitive form state in host telemetry.
