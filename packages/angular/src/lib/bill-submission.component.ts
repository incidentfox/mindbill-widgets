import { FeeDetails, billSubmissionCalculationContext, billSubmissionFeeRequest, billSubmissionQuoteContext, billSubmissionUsesSharedDiagnoses, billSubmissionLineDiagnosisCodes, setBillSubmissionDiagnosisAssignments, remapBillSubmissionDiagnoses, setBillSubmissionSharedDiagnoses, treatmentDraftKey, isMedicalLegalCode, isProlongedCode } from "./submission-treatment";
  ...
  template: `
    <form class="mbs" [ngStyle]="themeStyle" (submit)="submit($event)" novalidate>
      ...
      <fieldset id="mbs-lines" [class.invalid-set]="bad('serviceLines')"><legend>Service lines</legend>
        @if (isTreatment) { <label class="checkbox-label"><input type="checkbox" name="sharedDiagnoses" [ngModel]="sharedDiagnoses" (ngModelChange)="setSharedDiagnoses($event)" /> Use the same diagnoses for all services (up to four)</label><p>{{ procedureStatus }}</p> }
        ...
        @for (line of bill.serviceLines; track $index; let index = $index) { <div class="service-line">
          <div class="cell"><span class="mobile-label">Procedure</span>
            <mindbill-combo-box [ariaLabel]="'Procedure code ' + (index + 1)" [value]="line.code" placeholder="Search or enter code…" [options]="procedureComboOptions" [createOption]="customProcedureOption" (queryChange)="searchProcedures($event)" (selected)="selectProcedure(index, $event)"/>
            ...
            <div class="dx">
              @if (bill.diagnoses.length) {
                @for (code of bill.diagnoses; track code; let dxIndex = $index) {
                  <button type="button" [class.on]="hasDiagnosisPointer(line, dxIndex + 1)" [disabled]="isTreatment && sharedDiagnoses" [title]="dxLetter(dxIndex + 1) + ' — ' + code" [attr.aria-pressed]="hasDiagnosisPointer(line, dxIndex + 1)" [attr.aria-label]="'Point line ' + (index + 1) + ' at diagnosis ' + code" (click)="toggleDiagnosisPointer(index, dxIndex + 1)">{{ dxLetter(dxIndex + 1) }}</button>
                }
...
export class MindBillBillSubmissionComponent implements OnChanges, DoCheck, OnDestroy {
  ...
  searchProcedures(query: string) {
    if (!this.isTreatment) return;
    clearTimeout(this.procedureTimer); const generation = ++this.procedureGeneration;
    this.procedureStatus = "Searching procedures…";
    this.procedureTimer = setTimeout(async () => { try { const page = await this.referenceClient().searchProcedureCodes({ query, limit: 30 }); if (!this.destroyed && generation === this.procedureGeneration) { this.remoteProcedures = page.results.map(item => item.code); this.procedureStatus = this.remoteProcedures.length ? "" : "No matching procedures. You can enter a custom code."; } } catch { if (generation === this.procedureGeneration) this.procedureStatus = "Procedure search unavailable. You can enter a code."; } finally { if (!this.destroyed) this.changeDetector.markForCheck(); } }, 180);
  }
  setSharedDiagnoses(shared: boolean) {
    if (shared === this.sharedDiagnoses) return;
    if (shared) { if (this.bill.diagnoses.length > 4) { this.errorMessage = "Choose up to four diagnoses before sharing them across services."; return; } this.independentDiagnoses = this.bill.serviceLines.map((_, index) => billSubmissionLineDiagnosisCodes(this.bill, index)); this.bill = setBillSubmissionSharedDiagnoses(this.bill, this.bill.diagnoses); }
    else if (this.independentDiagnoses.length) this.bill = setBillSubmissionDiagnosisAssignments(this.bill, this.bill.serviceLines.map((_, index) => this.independentDiagnoses[index] ?? this.bill.diagnoses));
    this.sharedDiagnoses = shared; this.billChange.emit(this.bill);
  }
  ngOnDestroy() { this.destroyed = true; ++this.procedureGeneration; clearTimeout(this.quoteTimer); clearTimeout(this.procedureTimer); clearTimeout(this.searchTimer); clearTimeout(this.postalTimer); }
  ...
  get procedureComboOptions(): MindBillComboOption[] {
    return [...this.procedureOptions.map((item) => ({ id: item.code, label: item.code, detail: item.description })), ...this.remoteProcedures.filter(code => !this.procedureOptions.some(item => item.code === code)).map(code => ({ id: code, label: code, detail: "Procedure code" }))];
  }
  ...
  addDiagnosis(item: BillDiagnosisCode): void { if (!this.hasDiagnosis(item.code) && this.bill.diagnoses.length < (this.sharedDiagnoses ? 4 : 12)) this.bill.diagnoses.push(item.code); if (this.sharedDiagnoses) this.bill = setBillSubmissionSharedDiagnoses(this.bill, this.bill.diagnoses); this.diagnosisQuery = ""; this.diagnosisResults = []; this.invalidFields.delete("diagnoses"); this.billChange.emit(this.bill); }
  removeDiagnosis(code: string): void { this.bill = remapBillSubmissionDiagnoses(this.bill, this.bill.diagnoses.filter((item) => item !== code)); this.billChange.emit(this.bill); }
  ...
  toggleDiagnosisPointer(index: number, pointer: number): void {
    const line = this.bill.serviceLines[index]!;
    if (this.sharedDiagnoses) return;
    if (!this.hasDiagnosisPointer(line, pointer) && (line.diagnosisPointers?.length ?? 0) >= 4) return;
