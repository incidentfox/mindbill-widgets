  ...
  return <section className={["mb-readonly-bill", className].filter(Boolean).join(" ")} style={mindBillAppearanceStyle(appearance, style)} aria-label="Bill details">
    ...
    <BillDetailSection title="Service lines" validationIssues={validationIssues?.services ?? []}><div className="mb-read-lines" role="table" aria-label="Service lines">
      ...
      {bill.lineItems.map((line, index) => <div className="mb-read-line" role="row" key={line.id ?? `${line.code}-${index}`}>
        <strong>{line.code}</strong><div className="mb-read-chips">{line.modifiers.length ? line.modifiers.map((modifier) => <span key={modifier}>{modifier}</span>) : "—"}</div><span>{line.units}</span><details className="mb-read-charge"><summary aria-label={`Charge breakdown for ${line.code}, line ${index + 1}`}>{money(line.charge)}</summary><div className="mb-read-charge-detail"><strong>{line.code} · Charge breakdown</strong><dl><Value label="Units">{line.units}</Value>{line.units > 0 && Number.isFinite(line.charge / line.units) ? <Value label="Average charge per unit">{new Intl.NumberFormat(undefined, { style: "currency", currency: "USD", maximumFractionDigits: 4 }).format(line.charge / line.units)}</Value> : null}<Value label="Total charge">{money(line.charge)}</Value>{line.feeSchedule != null ? <Value label="Recorded fee schedule">{money(line.feeSchedule)}</Value> : null}</dl><p>Average calculated from the saved charge and units; this is not a base rate or modifier calculation. {line.feeSchedule == null ? "No fee schedule amount was recorded." : "Fee schedule shown as recorded for this line."}</p></div></details>
      </div>)}
