...
export function ConnectedBillLifecycle({ appearance, sandboxControls = false, className, style, loadingFallback, errorFallback, onChanged, ...options }: ConnectedBillLifecycleProps): ReactElement {
  ...
  return <section className={["mb-connected-lifecycle", className].filter(Boolean).join(" ")} style={mindBillAppearanceStyle(appearance, style)}>
    ...
    {panel === "submit_new_bill" && correctionInitialBill ? <LifecycleDialog title="Submit New Bill" wide onClose={() => setPanel("")}><section className="mb-lifecycle-correction"><header><div><h3>Submit New Bill</h3><p>This closed bill stays closed and keeps its record. Review the carried-over snapshot below and submit a fresh bill — both bills stay linked in the submissions timeline.</p></div><button type="button" className="mb-lifecycle-button secondary" onClick={() => setPanel("")}>Cancel</button></header><label className="mb-lifecycle-correction-note"><span>Submission note (optional)</span><textarea value={reason} placeholder="Why is a new bill being submitted?" onChange={(event) => setReason(event.target.value)} /></label><BillSubmissionForm
      initialBill={correctionInitialBill}
      attachments={correctionAttachments}
    ...
    {panel === "send_duplicate" ? (duplicateDelivery
      ? <SendRouteDialog
        ...
        onCancel={() => setPanel("")}
        onConfirm={(submission) => void complete("Duplicate bill sent.", () => lifecycle.sendDuplicateBill(submission))}
      />
    ...
    {panel === "report_status" ? <ReportBillStatusDialog
      ...
      {...(receiptEntries.length ? { receipt: <BillHistoryTable entries={receiptEntries} {...(appearance ? { appearance } : {})} /> } : {})}
      submitting={lifecycle.isMutating}
      error={lifecycle.error?.message ?? null}
      onCancel={() => setPanel("")}
      onSave={(input) => void complete("Bill status reported.", () => lifecycle.reportBillStatus(input))}
      {...(appearance ? { appearance } : {})}
    /> : null}
