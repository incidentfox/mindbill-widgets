...
import { defaultNotificationPreferences, notificationSettingsUpdate, type NotificationAgingDay, type NotificationReportDigest } from "./notification-settings";
...
export type NotificationRecipientInvitation = {
  ...
  quietHours: boolean;
  /** Optional for existing hosts; omitted means off. */
...
export function notificationRecipientInvitation(input: Omit<NotificationRecipientInvitation, "requestId">, requestId: string): NotificationRecipientInvitation {
  ...
  return { requestId, email: input.email.trim().toLowerCase(), audience: input.audience,
    ...
    reportDigest: preferences.reportDigest ?? "off" };
}

const css = `
.mbnr-digest{margin-top:12px}.mbnr-digest small{display:block;margin-top:6px}
...
function recipientCategories(recipient: NotificationRecipient): string {
  return [recipient.statusUpdates ? "Status and payment updates" : "",
    recipient.agingDays.length ? `${recipient.agingDays.join(" / ")} day reminders` : "",
    recipient.reportDigest === "daily" ? "Daily activity digest" : recipient.reportDigest === "weekly" ? "Weekly activity digest" : "",
  ].filter(Boolean).join(" · ") || "No email categories";
}
...
function RecipientSettingsSession({ adapter, appearance, className, style }: NotificationRecipientsSettingsProps): ReactElement {
  ...
  const canInvite = !busy && !!snapshot?.available && !!email.trim() && (preferences.statusUpdates || preferences.agingDays.length > 0 || (preferences.reportDigest ?? "off") !== "off");
    ...
    <style>{css}</style><div className="mbnr-card">
      ...
      <form className="mbnr-form" onSubmit={event => {
        ...
        <fieldset disabled={busy || !snapshot?.available}><legend>Alerts to request</legend>
          ...
          <label className="mbnr-digest"><span>Scheduled activity digest</span><select value={preferences.reportDigest ?? "off"} onChange={event => setPreferences(value => ({ ...value, reportDigest: event.target.value as NotificationReportDigest }))}><option value="off">Off</option><option value="daily">Daily</option><option value="weekly">Weekly</option></select><small className="mbnr-muted">Activity counts at 9 am Pacific, every day or every Monday. No financial reports or attachments.</small></label>
        </fieldset>
        <p className="mbnr-muted">Emails contain general alerts and activity counts, not patient details, reports or attachments. The recipient reviews these choices and can unsubscribe at any time.</p>
