...
function LifecycleContent({ rfa: providedRfa, options, permissions = [], onUpdated, ...appearance }: Omit<RfaLifecycleControlsProps, keyof OrganizationClientOptions> & { options: OrganizationClientOptions }): ReactElement {
  ...
  return <TreatmentDraftShell {...appearance} title="Receipt and utilization review" description="Record evidence received from the claims administrator and track each requested treatment. Dates and deadlines come from MindBill.">
    <div style={{ display: "grid", gap: 16 }}>
      ...
      {act && !rfa.receivedAt && !["draft", "ready", "canceled", "closed"].includes(rfa.status) ? <details><summary>Record confirmed receipt</summary><form onSubmit={event => submit(event, "receipt", data => ({ channel: text(data, "channel") as RfaReceiptInput["channel"], receivedAt: timestamp(data, "receivedAt"), ...(text(data, "proofDocumentId") ? { proofDocumentId: text(data, "proofDocumentId") } : {}), ...(text(data, "providerMessageId") ? { providerMessageId: text(data, "providerMessageId") } : {}) }), (input, key) => client.recordReceipt(rfa.id, input, key))}><fieldset disabled={disabled}><legend>Receipt evidence</legend><p>A sent fax alone does not establish receipt. Enter the administrator's confirmed receipt time and a proof document or delivery reference.</p><div className="mbtd-grid"><label>Confirmed receipt time<input type="datetime-local" name="receivedAt" required /></label><label>Receipt channel<select name="channel">{["fax", "email", "edi", "portal", "mail", "manual"].map(value => <option key={value}>{value}</option>)}</select></label>{documentSelect("proofDocumentId", "Receipt proof", undefined, false)}<label>Delivery or provider reference<input name="providerMessageId" /></label></div><button type="submit">Save confirmed receipt</button></fieldset></form></details> : null}
      {act && decisionAllowed && pendingItems.length ? <details><summary>Record treatment decisions ({pendingItems.length} pending)</summary><form onSubmit={event => submit(event, "decisions", data => {
      ...
      }, (input, key) => client.recordDecisions(rfa.id, input, key))}><fieldset disabled={disabled}><legend>Treatment decisions</legend><p>Record only treatments addressed by this response. Unselected treatments remain pending.</p><div className="mbtd-grid"><label>Decision time<input type="datetime-local" name="decidedAt" required /></label>{documentSelect("responseDocumentId", "Utilization review response", ["ur_response"])}{documentSelect("imrDocumentId", "Independent medical review form", ["imr_form"], false)}</div>
      ...
      <details open><summary>Follow-up tasks</summary>{taskError ? <p role="alert">{taskError}</p> : null}{loading ? <p role="status">Loading follow-ups…</p> : tasks.length ? tasks.map(task => <FollowUpForm key={`${task.id}:${task.updatedAt}`} task={task} editable={act} disabled={Boolean(disabled)} onSave={input => run(`follow-up:${task.id}`, input, async key => { await client.updateFollowUp(task.id, input, key); return records.get(rfa.id); })} />) : !taskError ? <p>No follow-up tasks are recorded for this request.</p> : null}<button type="button" disabled={loading || disabled} onClick={() => setReload(value => value + 1)}>Refresh follow-ups</button></details>
      <p className="mbtd-note">Need to correct a recorded decision or review an exceptional decision clock? These require the explicit <a href="https://docs.mindbill.org/guides/rfas" target="_blank" rel="noopener noreferrer">RFA API review workflows</a>. Existing evidence remains in the request history.</p>
    </div>
  </TreatmentDraftShell>;
}
function FollowUpForm({ task, editable, disabled, onSave }: { task: RfaFollowUp; editable: boolean; disabled: boolean; onSave: (input: RfaFollowUpUpdate) => Promise<void> }): ReactElement {
  const [review, setReview] = useState("");
  return <fieldset disabled={disabled}><legend>{label(task.kind)} · {label(task.status)}</legend><p>Due {displayDate(task.snoozedUntil ?? task.dueAt)}{task.responseFilename ? ` · ${task.responseFilename}` : ""}</p>{task.lastNote ? <p>{task.lastNote}</p> : null}{editable && !task.resolvedAt ? <form onSubmit={event => {
    event.preventDefault(); const data = new FormData(event.currentTarget); const note = text(data, "note");
    if (!note) return;
    const input: RfaFollowUpUpdate = review ? { note, responseReview: review === "no_new_decision" ? { disposition: "no_new_decision", noNewDecisionConfirmed: true } : { disposition: "decisions_recorded", allDecisionsRecordedConfirmed: true } } : { note, outcome: text(data, "outcome") as NonNullable<RfaFollowUpUpdate["outcome"]>, snoozedUntil: timestamp(data, "snoozedUntil") };
    void onSave(input);
  }}><div className="mbtd-grid">{task.responseDocumentId ? <label>Response review<select value={review} onChange={event => setReview(event.target.value)}><option value="">Record follow-up contact</option><option value="no_new_decision">Confirm no new treatment decision</option><option value="decisions_recorded">Confirm all decisions have been recorded</option></select></label> : null}{!review ? <><label>Contact outcome<select name="outcome">{["message_left", "decision_pending", "decision_issued", "not_on_file", "reviewed"].map(value => <option key={value} value={value}>{label(value)}</option>)}</select></label><label>Next follow-up<input type="datetime-local" name="snoozedUntil" required /></label></> : <label><input type="checkbox" required /> {review === "no_new_decision" ? "I reviewed the response and confirm it contains no new treatment decision." : "I confirm every treatment decision in the response has been recorded."}</label>}<label className="mbtd-wide">Follow-up note<textarea name="note" maxLength={2000} required /></label></div><button type="submit">Save follow-up</button></form> : null}</fieldset>;
}
