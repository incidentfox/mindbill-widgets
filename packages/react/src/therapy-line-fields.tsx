...
export const isTherapyEditorCode = (code: string) => code.trim().toUpperCase() === "97110";
...
export function missingTherapyDetails(details: TherapyInputs): string[] {
...
export function therapyCalculationContext(placeOfService: string, details: TherapyInputs, saved: BillFeeContext = {}): BillFeeContext {
  if (missingTherapyDetails(details).length) return {};
  const facts = details.therapy!;
  ...
  return {
    ...
    therapyContext: {
      ...facts,
      providerKind: details.providerKind as CaTherapyContext["providerKind"],
      ...
      placeOfService, directOneOnOneMinutes: details.minutes!, totalVisitMinutes: details.totalMinutes!, visitsOnDate: facts.visitsOnDate!,
...
export function TherapyLineFields({ index, details, update }: { index: number; details: TherapyDetails; update: (patch: Partial<TherapyDetails>) => void }) {
  const facts = details.therapy ?? {};
  ...
  return <div className="mbsf-grid" aria-label={`Therapy service details for line ${index + 1}`}>
    ...
    <p className="mbsf-help mbsf-span">Include all services for this patient and service date by the provider or group in this bill. Use actual documented minutes. Other arrangements, additional same-day services and some timed-unit combinations require fee review.</p>
  </div>;
}
