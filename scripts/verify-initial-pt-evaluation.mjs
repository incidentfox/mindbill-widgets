...
const bill={externalId:'synthetic_therapy',billingMode:'professional',patient:{firstName:'Ada',lastName:'Example',dateOfBirth:'1980-01-02',address},claim:{claimNumber:'SYNTHETIC-7',employer:'Synthetic Foods',dateOfInjury:'2026-08-01',claimsAdministrator:{id:'payer_synthetic',name:'Synthetic Claims Administrator'}},service:{date:'2026-09-17'},billingProvider:{name:'Synthetic Medical Group',taxId:'123456789',npi:'1234567890',phone:'2135550100',address},renderingProvider:{name:'Ada Therapist',npi:'1098765432',taxonomy:'225100000X'},serviceLocation:{name:'Synthetic Office',placeOfServiceCode:'11',address},diagnoses:['M79.641'],serviceLines:[{code:'97161',units:1,serviceDate:'2026-09-17',diagnosisPointers:[1],modifiers:['GP']}]};
window.calls=[];
const onQuoteFee=async(input)=>{
 window.calls.push(input);
 return input.hasFeeAgreement || input.therapyContext?.priorInitialEvaluationInEpisode !== false
 ? {status:'requires_review',reason:'therapy_no_fee_agreement_confirmation_required',provenance:[]}
 : {status:'priced',amountCents:12345,scheduleMaximumCents:12345,basis:'ca_omfs_therapy',provenance:[],notes:[]};
};
const e=React.createElement;
createRoot(document.getElementById('app')).render(e(BillSubmissionForm,{initialBill:bill,treatmentBilling:true,onQuoteFee,appearance:{preset:'mindbill'},deliveryRoutePicker:'off',procedureOptions:[{code:'97161',description:'Initial PT evaluation'}]},e(BillSubmissionServiceLinesSection)));
...
try {
  ...
  for (const [label, value] of [["Therapy visits", "1"]]) {
    await page.getByLabel(`${label} for line 1`, { exact: true }).fill(value);
  }
  assert.equal(await page.getByLabel("Direct one-on-one minutes for line 1", { exact: true }).count(), 0);
  await page.getByLabel("Prior initial evaluation for line 1", { exact: true }).selectOption("false");
  await page.waitForFunction(() => document.querySelector(".mbsf-money")?.textContent === "$123.45");
  const request = await page.evaluate(() => window.calls.at(-1));
  assert.equal(request.hasFeeAgreement, false);
  assert.deepEqual(request.therapyContext, { providerKind: "physical_therapist", personallyPerformed: true, assistantInvolved: false, hospitalPatient: false, incidentToPhysicianService: false, globalPeriodApplies: false, hpsaBonusEligible: false, placeOfService: "11", priorInitialEvaluationInEpisode: false, visitsOnDate: 1, completeSameDayServices: true, otherSameDayServices: false });
  const count = await page.evaluate(() => window.calls.length);
  await page.getByLabel("Prior initial evaluation for line 1", { exact: true }).selectOption("");
  await page.waitForTimeout(350);
  ...
  for (const [name, width] of [["desktop", 1360], ["mobile", 390], ["small-mobile", 320]]) {
    ...
    await page.screenshot({ path: resolve(output, `initial-pt-${name}.png`), fullPage: true });
  }
  await page.getByLabel("Prior initial evaluation for line 1", { exact: true }).selectOption("true");
  await page.waitForFunction(() => window.calls.at(-1)?.therapyContext?.priorInitialEvaluationInEpisode === true);
  assert.equal(await page.locator(".mbsf-money").first().textContent(), "Needs review");
