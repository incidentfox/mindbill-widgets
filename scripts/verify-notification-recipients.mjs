...
import {NotificationRecipientsSettings} from './packages/react/dist/index.js';
...
window.fixture={attempts,gateOff:()=>{available=false;}};
createRoot(document.getElementById('app')).render(React.createElement(NotificationRecipientsSettings,{identityKey:'synthetic-practice',adapter,appearance:{preset:'mindbill'}}));
...
try {
 ...
 await page.getByLabel("Bill status and payment updates").check();
 ...
 console.log(JSON.stringify({passed:true,checks:["default off","assigned scope","idempotent retry","pending off","mobile overflow","gate-off disable"],screenshots:output}));
